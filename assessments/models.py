...
class Test(models.Model):
    ...
    start_date = models.DateTimeField(
        null=True, blank=True, help_text="Test start date and time")
    end_date = models.DateTimeField(
        null=True, blank=True, help_text="Test end date and time")
    allow_multiple_attempts = models.BooleanField(default=False)
    max_attempts = models.PositiveIntegerField(
        null=True,
        blank=True,
        help_text="Maximum number of attempts (if multiple attempts allowed)"
    )
    time_limit_minutes = models.PositiveIntegerField(
        null=True,
        blank=True,
        help_text="Time limit in minutes for completing the test once started. "
                  "If not set, there is no per-attempt time limit."
    )
...
class Question(models.Model):
    ...
    correct_answer_text = models.TextField(
        null=True, blank=True, help_text="Expected answer for open questions")
    sample_answer = models.TextField(
        null=True, blank=True, help_text="Sample answer for reference")
    key_words = models.TextField(
        null=True, blank=True, help_text="Comma-separated keywords for automatic grading of open questions")

    # For matching questions
    matching_pairs_json = models.JSONField(
        null=True, blank=True, help_text="JSON array of matching pairs")
...
class Attempt(models.Model):
    ...
    student = models.ForeignKey(
        "users.User", on_delete=models.CASCADE, related_name="test_attempts")
    ...
    percentage = models.FloatField(null=True, blank=True, validators=[
    ...
    def is_time_limit_exceeded(self) -> bool:
        """
        Check whether the time limit for this attempt has been exceeded.

        Returns:
            bool: True if the test has a time limit and the current time is past
                  started_at + time_limit_minutes. False otherwise.
        """
        ...
        deadline = self.started_at + \
            timedelta(minutes=self.test.time_limit_minutes)
...
class Answer(models.Model):
    ...
    selected_options = models.ManyToManyField(
        Option, blank=True, related_name="selected_in_answers")

    # For open questions
    text_answer = models.TextField(null=True, blank=True)

    # For matching questions
    matching_answers_json = models.JSONField(
        null=True, blank=True, help_text="JSON array of matching answers")
    ...
    def calculate_score(self):
        ...
        if self.question.type == QuestionType.MULTIPLE_CHOICE:
            ...
            selected_correct = self.selected_options.filter(
            ...
            selected_correct = self.selected_options.filter(
                is_correct=True).count()
            selected_incorrect = self.selected_options.filter(
            ...
            if self.question.key_words and self.text_answer:
                ...
                keywords = [kw.strip().lower()
                            for kw in self.question.key_words.split(',') if kw.strip()]
            ...
            if self.question.correct_answer_text:
                ...
                expected = " ".join(
                    self.question.correct_answer_text.strip().lower().split())
            ...
            if len(correct_pairs_set) > 0:
                ...
                # Partial credit: proportion of correct pairs minus penalty for incorrect
                    ...
                    score_ratio = (correct_count / len(correct_pairs_set)) - \
                        (incorrect_count * 0.25 / len(correct_pairs_set))
